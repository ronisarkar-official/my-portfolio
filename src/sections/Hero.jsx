...
/* Move static SVGs out so they aren't recreated every render */
...
const Hero = () => {
  ...
  // memoize the sizes calculation so it only recalculates when inputs change
  const sizes = useMemo(() => calculateSizes(isSmall, isMobile, isTablet), [isSmall, isMobile, isTablet]);

  // memoize camera position array (prevents creating new array each render)
  const cameraPosition = useMemo(() => [0, 0, isMobile ? 25 : 30], [isMobile]);

  /* Memoize the 3D scene subtree so React only rebuilds it when sizes or isMobile changes.
     This reduces JSX allocations and the tree reconciliation cost. Behavior/ordering preserved. */
    ...
    <section className="min-h-screen w-full flex flex-col relative" id="home">
      ...
      <div className="w-full h-full absolute inset-0">
        <Canvas className="w-full h-full">
          <Suspense>{SceneContents}</Suspense>
        </Canvas>
      </div>
